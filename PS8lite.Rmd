...
* 36 seems to be a large number of variables to predict one outcome, and there are some variables that overlap. Also becasue making R run through `mtry` between 2 to 36 takes forever, I decided to use 15 as an arbitrary number of measures to find the optimal number of predictor variables. 
* From the results of 10-fold cross validation on 15 features (out of 36), mtry = 14 yielded lowest RMSE. Since the cross validation takes more than 10 minutes to run, I didn't set it to run when knitting.
...
![](images/screenshot.jpg){ width=100% }
